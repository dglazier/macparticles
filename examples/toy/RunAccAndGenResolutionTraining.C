{
  ...
  DataWithAccAndGenFiles  dl("simtree", "acc_training.root", "gen_training.root");
  ...
  config.Load("fast_simulation_accandgen/");
 ...
 auto h0 = accEvents.Histo2D({"DPvP","#DeltaP v P",100,0,10,100,-2,4},"truP","diffP");
  ...
  auto h1 = accEvents.Histo1D({"diffTheta","#theta",100,-15,15},"diffTheta");
  new TCanvas();
  h1->DrawCopy();
  auto h2 = accEvents.Histo1D({"Phi","#phi",100,-180,180},"recon.recPhi");
  new TCanvas();
  h2->DrawCopy();

  auto h3 = accEvents.Histo2D({"DPhiVTh","#phi v theta",100,0,180,100,-20,20},"truTheta","diffPhi");
  new TCanvas();
  h3->DrawCopy();
  
  auto h4 = accEvents.Histo2D({"DPhiVTh","#phi v theta",100,0,180,100,-20,20},"truTheta","diffRealPhi");
  new TCanvas();
  h4->DrawCopy();

  auto h6 = accEvents.Histo1D({"DPhir","#phi v theta",100,-25,25},"diffRealPhi");
  new TCanvas();
  h6->SetLineColor(2);
  h6->DrawCopy("same");
  auto h5 = accEvents.Histo1D({"DPhif","#phi v theta",100,-25,25},"diffPhi");
  new TCanvas();
  h5->DrawCopy();

  auto h7 = accEvents.Histo2D({"DPhiDThr","D #phi v theta",100,-25,25,100,-15,15},"diffPhi","diffTheta");
  new TCanvas();
  h7->DrawCopy("col");
  auto h8 = accEvents.Histo2D({"DPhiDThf","D #phi v theta",100,-25,25,100,-15,15},"diffRealPhi","diffRealTheta");
  new TCanvas();
  h8->DrawCopy("col");
